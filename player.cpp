...
void PLAYER::think()
{
	...
	if (life>0)
	{
		if (RIGHT&&get_Ctype(chip_num.x + 1, chip_num.y) != STAGE_TYPE::Wall)
		{
		...
		if (LEFT &&get_Ctype(chip_num.x - 1, chip_num.y) != STAGE_TYPE::Wall)
		{
		...
		if (UP   &&get_Ctype(chip_num.x, chip_num.y - 1) != STAGE_TYPE::Wall)
		{
		...
		if (DOWN &&get_Ctype(chip_num.x, chip_num.y + 1) != STAGE_TYPE::Wall)
		{
